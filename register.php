...
$sql="SELECT id, Approved FROM $_SESSION[dbname].requests WHERE email='$email'";     //verifica daca email-ul se afla in requests
...
$sql="SELECT id FROM $_SESSION[dbname].users WHERE email='$email'";      //verifica daca email-ul se afla in users
if($result=$conn->query($sql)){
    $row=$result->fetch_assoc();
    if($row["id"]){                                         //se afla
...
$sql="SELECT MAX(id) FROM $_SESSION[dbname].users";
if(!($result=$conn->query($sql)))
...
$sql="INSERT INTO $_SESSION[dbname].users(id,Nume,username,email,Parola,Rol) VALUES('$id','$name','$username','$email','$pass','User')";
if(!($result=$conn->query($sql))){
    die("Failed to connect: ".$conn->connect_error);
}

$sql="SELECT MAX(id) FROM $_SESSION[dbname].members";
if(!($result=$conn->query($sql)))
    die("Failed to connect: ".$conn->error);
else{
    $row=$result->fetch_array();
...
$sql="INSERT INTO $_SESSION[dbname].members(id,Nume,Departament,Extern,EchipeFR,Granturi,International,Title,Score,Gender) VALUES('$id','$name',0,0,0,0,0,'BABY LION',0,'$gender')";
if(!$result=$conn->query($sql))
    die("Failed to connect: ".$conn->connect_error);

$sql="DELETE FROM $_SESSION[dbname].requests WHERE Email='$email'";
if(!$result=$conn->query($sql))
    die("Failed to connect: ".$conn->connect_error);
